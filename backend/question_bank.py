...
_add("code_javascript_async", "Coding — JavaScript / Async", "Tech & Eng",
    ...
    {"id": "q2", "type": "code", "weight": 4, "language": "javascript",
     ...
     "test_code": "const out = await promisePool([1,2,3,4,5], async (n) => { await new Promise(r=>setTimeout(r,5)); return n*n; }, 2);\nassert.deepStrictEqual(out, [1,4,9,16,25]);\nconst empty = await promisePool([], async (x)=>x, 3);\nassert.deepStrictEqual(empty, []);\n"},
    {"id": "q3", "type": "code", "weight": 4, "language": "javascript",
     ...
     "test_code": "let debCount=0; const d = debounce(()=>debCount++, 30);\nd(); d(); d();\nawait new Promise(r=>setTimeout(r,60));\nassert.strictEqual(debCount, 1, 'debounce should fire once after quiet period');\nlet thCount=0; const t = throttle(()=>thCount++, 30);\nt(); t(); t();\nawait new Promise(r=>setTimeout(r,10));\nassert.strictEqual(thCount, 1, 'throttle leading-edge fires once during window');\n"},
])
